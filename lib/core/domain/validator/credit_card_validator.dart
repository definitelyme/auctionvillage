...
mixin _CreditCardValidator {
  ...
  static StringValidator<String> validateCardCVV(String? input) {
    ...
    return _onlyDigits(
      ...
      if (val.length < 3 || val.length > 4)
        return left(
          FieldObjectException.invalid('Invalid CVV!'),
        );
      return right(input);
    });
  }
  ...
  static Either<FieldObjectException<String>, String> validateCardExpiryDate(
    ...
    if (month == null || year == null) return left(FieldObjectException.invalid('Invalid card expiry date!'));

    if (clean.length > 1 && (month < 1) || (month > 12)) {
      // A valid month is between 1 (January) and 12 (December)
      return left(FieldObjectException.invalid('Invalid Month.'));
    }

    if (!clean.isNullOrBlank && clean[clean.length - 1] == '/') return left(FieldObjectException.invalid('Expiry year is required'));
    ...
    if (fourDigitsYear != null && ((fourDigitsYear < 1) || (fourDigitsYear > 2099))) {
      ...
      return left(FieldObjectException.invalid('Expiry year is invalid.'));
    }

    if (!hasDateExpired(month, year)) {
      return left(FieldObjectException.invalid('Expired Card!'));
    }
  ...
  static Either<FieldObjectException<String>, String> _onlyDigits(
    ...
    if (!containsOnlyDigits) return left(FieldObjectException.invalid(message));
  ...
  static Either<FieldObjectException<String>, String> _validateWithLuhnAlgorithm(String input) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      if (digit == null) return left(FieldObjectException.invalid(INVALID_CARD_NUMBER));
    ...
    if (sum % 10 != 0) {
      return left(FieldObjectException.invalid(INVALID_CARD_NUMBER));
    }
