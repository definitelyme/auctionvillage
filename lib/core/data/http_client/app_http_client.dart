...
class AppHttpClient extends DioForNative {
  ...
  @override
  Future<Response<T>> fetch<T>(RequestOptions requestOptions) async => _mapException(() => _client.fetch(requestOptions));
  ...
  Future<Response<T>> _mapException<T>(HttpLibraryMethod<T> method) async {
    try {
      ...
      if (e.error is SocketException || e.message.contains('SocketException'))
        throw AppNetworkException(reason: AppNetworkExceptionReason.socketException, exception: e);
      ...
      switch (e.type) {
        ...
        case DioErrorType.response:
          ...
          throw _mapper!.call(response) ?? AppNetworkResponseException(exception: e, code: response.statusCode, data: response.data);
