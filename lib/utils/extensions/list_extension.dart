...
extension KtX2<T> on KtIterable<T> {
  ...
  int? indexOfFirstOrNull(bool Function(T) predicate) {
    try {
      final index = indexOfFirst(predicate);
      if (index == -1) return null;
      return index;
    } catch (_) {
      return null;
  ...
  int? indexOfLastOrNull(bool Function(T) predicate) {
    try {
      final index = indexOfLast(predicate);
      if (index == -1) return null;
      return index;
    } catch (_) {
      return null;
